...
namespace IQeSign.TicketBai.Http;

/// <summary>
/// Cliente HTTP interno que encapsula la comunicación con la API IQ eSign TicketBAI.
...
internal sealed class IQeSignHttpClient
{
    ...
    /// <summary>Realiza una petición PUT autenticada sin cuerpo.</summary>
    public Task<TResponse> PutAsync<TResponse>(string relativeUrl, CancellationToken ct = default)
        where TResponse : ApiResponse
        => SendAsync<TResponse>(HttpMethod.Put, relativeUrl, body: null, ct);
        ...
        where TResponse : ApiResponse
    {
        ...
        _logger.LogDebug("IQeSign TicketBAI API → {Method} {Url}", method, relativeUrl);
        ...
        {
            _logger.LogError(ex, "Error de comunicación con la API IQ eSign TicketBAI ({Method} {Url})", method, relativeUrl);
            throw;
        }
    ...
    {
        _logger.LogDebug("Solicitando token JWT a la API IQ eSign TicketBAI...");
        ...
        if (!loginResult.IsSuccess || loginResult.Result?.Token is null)
            throw new IQeSignAuthException(
                $"Error al autenticar en IQ eSign TicketBAI API. Código: {loginResult.ErrorCode}. Mensaje: {loginResult.ErrorMessage}");
        ...
        where TResponse : ApiResponse
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new IQeSignApiException(
                $"La API IQ eSign TicketBAI devolvió HTTP {(int)response.StatusCode} en {method} {url}. Respuesta: {content}",
                (int)response.StatusCode);
        }

        var result = JsonSerializer.Deserialize<TResponse>(content, JsonOptions);
        return result ?? throw new IQeSignApiException(
            $"La respuesta de la API IQ eSign TicketBAI no pudo deserializarse ({method} {url}).", (int)response.StatusCode);
    }
